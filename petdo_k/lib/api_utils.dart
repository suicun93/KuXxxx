...
class ApiConnect extends GetConnect{
  ...
  Future<Response<T>> callDogApi<T>(String endpoint, Decoder<T>? decoder) {
    return get(
        'https://api.thedogapi.com/v1/$endpoint',
        decoder: decoder,
...
class ApiUtils {
  ...
  static Future<Response<T>> getCatData<T>(String endpoint, Decoder<T>? decoder) {
    _apiConnect ??= ApiConnect();
    return _apiConnect!.callCatApi(endpoint, decoder);
  }

  static Future<Response<T>> getDogData<T>(String endpoint, Decoder<T>? decoder) {
    _apiConnect ??= ApiConnect();
